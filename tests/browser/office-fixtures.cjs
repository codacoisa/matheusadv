...
function zipStored(entries) {
  const normalizedEntries = entries.map(([name, value]) => [name, Buffer.isBuffer(value) ? value : Buffer.from(value)]);
  const localParts = [];
  const centralParts = [];
  let offset = 0;
  for (const [name, data] of normalizedEntries) {
...
function minimalDocx(text = 'OfficeJur WASM') {
  const documentText = Array.isArray(text)
    ? text.map((segment) => `<w:r><w:t>${segment}</w:t></w:r>`).join('')
  ...
  const entries = [
    ...
    ['word/document.xml', `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p>${documentText}</w:p><w:sectPr/></w:body></w:document>`],
  ];
  return zipStored(entries);
}

function minimalXlsx(text = 'OfficeJur XLSX') {
  return zipStored([
    ...
    ['xl/worksheets/sheet1.xml', `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>${text}</t></is></c></row></sheetData></worksheet>`],
  ]);
}

function minimalPptx(text = 'OfficeJur PPTX') {
  return zipStored([
