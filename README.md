...
Dados jurídicos e financeiros não são arquivos da aplicação e não devem ser versionados. A área permanente `/configuracoes/` concentra o Gist ID, o token e a sincronização automática de um único Gist privado no navegador. Os módulos sincronizáveis não possuem controles próprios de Gist: apenas consomem essa configuração compartilhada e armazenam seus dados em arquivos fixos e separados. Ao abrir, cada módulo baixa, mescla e publica seu próprio arquivo. O token do GitHub permanece somente no navegador, e credenciais privadas de integrações, como o Mercado Pago, permanecem em serviços protegidos.
