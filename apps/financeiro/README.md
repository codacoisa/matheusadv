...
Os dados ficam no navegador e podem ser sincronizados com o Gist secreto global definido na área **Configurações** do OfficeJur. Gist ID, token e sincronização automática valem para todos os módulos sincronizáveis no mesmo navegador e não podem ser alterados dentro do Financeiro. O módulo usa `financeiro-juridico.json` para os cadastros e `financeiro-arquivos.json` como índice leve de metadados. Cada PDF é codificado em Base64 e sincronizado em seu próprio payload `financeiro-pdf-<id>.b64`; ele só é baixado quando aberto ou baixado. Cada arquivo possui sua própria estratégia de mesclagem e registro de exclusões. O cache local dos PDFs usa IndexedDB para não disputar o limite do `localStorage` com os dados financeiros. Ao migrar o formato anterior, o arquivo agregado é preservado no Gist como `financeiro-arquivos-legado-v1.json`. O token do GitHub nunca é gravado no repositório: permanece apenas no navegador. Um Gist secreto não aparece em buscas, mas qualquer pessoa que obtenha sua URL poderá visualizar o conteúdo; Base64 não é criptografia.
