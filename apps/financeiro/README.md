...
Pessoas, clientes e casos possuem cadastros independentes. A lista de pessoas fica dentro da área de clientes. Uma pessoa física é cadastrada uma única vez e pode permanecer apenas como contato, ser promovida posteriormente a cliente ou representar uma ou mais pessoas jurídicas. O cliente pessoa jurídica possui razão social, CNPJ e dados próprios de contato e endereço, além de um ou vários representantes vinculados, com cargo, poderes, vigência e regra de assinatura. Um representante principal e ao menos um signatário são obrigatórios para clientes PJ. CPF e CNPJ não podem se repetir nos respectivos cadastros.
...
O formulário oferece **Melhorar ou sugerir com IA** para processos judiciais e casos administrativos, extrajudiciais ou consultivos. A ação usa o LLM7.io em modo anônimo e envia somente o título atual/rascunho, tipo, área e, quando disponíveis, classe e assuntos; cliente, partes fora do título, CPF/CNPJ, número CNJ, observações e movimentações não são enviados. A sugestão precisa trazer uma melhoria real de redação, pode resumir um qualificador de procedimento e deve preservar o núcleo do objeto. Quando o modelo de raciocínio esgota o limite sem produzir texto final, o módulo refaz a consulta com um limite maior. Respostas incompletas, excessivamente reduzidas ou que apenas trocam separadores/pontuação são rejeitadas e o título atual é mantido. Como o título digitado pode conter dados sensíveis, use a consulta anônima somente quando puder compartilhar esse conteúdo com serviço externo. A sugestão aceita é editável e precisa ser conferida antes do salvamento.
...
O módulo armazena cada domínio de forma independente: `financeiro-pessoas.json`, `financeiro-clientes.json`, `financeiro-casos.json`, `financeiro-pacotes.json`, `financeiro-equipe.json`, `financeiro-lancamentos.json`, `financeiro-cobrancas.json`, `financeiro-contas.json` e `financeiro-documentos.json`. Os relacionamentos usam IDs estáveis, por exemplo `personId`, `clientId`, `caseId`, `packageId` e `entryId`. Cada arquivo possui schema no namespace `officejur/`, versão numérica, data de atualização, registros ativos, exclusões e estratégia de mesclagem próprios. No navegador, os oito domínios operacionais ficam no IndexedDB `officejur-financeiro` e são gravados em uma única transação.
