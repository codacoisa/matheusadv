...
assert.match(app, /function enrichDataJudMunicipality/);
assert.match(app, /dataJudMunicipalityLabel/);
...
assert.match(app, /data-refresh-case-datajud-movements/);
assert.match(app, /dataJudMovementsSession = new Map/);
assert.match(app, /Movimentações consultadas temporariamente; nada foi salvo no caso/);
assert.match(app, /DATAJUD_PROCESS_NOT_FOUND/);
assert.match(app, /O preenchimento manual foi liberado para este cadastro/);
assert.doesNotMatch(html, /case-parties-editor|add-case-party|case-parties-rows/);
assert.doesNotMatch(app, /renderCaseParties|readCaseParties|data-case-party|parties:/);
assert.doesNotMatch(styles, /case-parties-editor|case-party-row/);
assert.match(datajudAssistant, /https:\/\/api-publica\.datajud\.cnj\.jus\.br/);
assert.match(datajudAssistant, /DATAJUD_PROXY_PATH/);
assert.match(html, /data-datajud-field="municipality"/);
