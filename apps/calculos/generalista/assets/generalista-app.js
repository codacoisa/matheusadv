(() => {
  ...
  function easyStep() {
    ...
    return `${basicFields(false)}<section class="form-section"><div class="section-heading"><div><h2>Critérios de atualização</h2><p class="hint">Defina os critérios em blocos separados para conferir rapidamente o que será aplicado.</p></div></div><div class="simple-criteria"><section class="criteria-card" aria-labelledby="simple-correction-title"><div class="criteria-card-head"><div><h3 id="simple-correction-title">Correção monetária</h3><p class="hint">Índice aplicado aos valores.</p></div><span class="criteria-kicker">Índice e pró-rata</span></div><div class="criteria-fields criteria-fields-correction"><div class="field"><label class="required" for="correctionType">Índice para corrigir os valores</label><select id="correctionType">${indexOptions(s.correctionType)}</select></div><label class="check criteria-check"><input id="correctionProrata" type="checkbox" ${s.correctionProrata ? "checked" : ""}><span>Correção pró-rata ${infoTip(correctionProrataHelp)}</span></label></div></section><section class="criteria-card" aria-labelledby="simple-interest-title"><div class="criteria-card-head"><div><h3 id="simple-interest-title">Juros</h3><p class="hint">Taxa e forma de incidência.</p></div><span class="criteria-kicker">Taxa e periodicidade</span></div><div class="criteria-fields criteria-fields-interest">${field("Taxa de juros (%)", "interestRate", s.interestRate, "number", "", false)}<div class="field"><label for="interestPeriodicity">Periodicidade</label><select id="interestPeriodicity">${periodicityOptions(s.interestPeriodicity)}</select></div><label class="check criteria-check"><input id="interestProrata" type="checkbox" ${s.interestProrata ? "checked" : ""}><span>Juros pró-rata ${infoTip(interestProrataHelp)}</span></label></div></section><section class="criteria-card criteria-card-wide" aria-labelledby="simple-charges-title"><div class="criteria-card-head"><div><h3 id="simple-charges-title">Encargos adicionais</h3><p class="hint">Multa e honorários aplicados ao resultado.</p></div><span class="criteria-kicker">Multa e honorários</span></div><div class="criteria-fields criteria-fields-charges">${field("Multa (%)", "penaltyRate", s.penaltyRate)}<label class="check criteria-check"><input id="penaltyOnInterest" type="checkbox" ${s.penaltyOnInterest ? "checked" : ""}><span>Multa incide sobre juros</span></label>${field("Valor dos honorários", s.feeType === "fixed" ? "feeAmount" : "feeRate", s.feeType === "fixed" ? s.feeAmount : s.feeRate, "number")}<div class="field"><label for="feeType">Tipo de aplicação</label><select id="feeType"><option value="percent" ${s.feeType !== "fixed" ? "selected" : ""}>Percentual (%)</option><option value="fixed" ${s.feeType === "fixed" ? "selected" : ""}>Fixo (R$)</option></select></div></div></section></div><div class="criteria-source"><div class="criteria-source-copy"><strong>Índices oficiais</strong><span class="hint">Os percentuais consultados são congelados no rascunho para auditoria.</span></div><button class="secondary" type="button" data-action="load-indices">${current.indexSnapshot ? "Atualizar índices oficiais" : "Carregar índices oficiais"}</button></div></section><section class="items-section form-section"><div class="card-title"><div><h2>Detalhamento dos itens</h2><p class="hint">Inclua débitos e abatimentos na ordem em que ocorreram.</p></div><button class="secondary" type="button" data-action="add-item">＋ Adicionar parcela</button></div>${itemsTable(false)}</section>`;
  }
