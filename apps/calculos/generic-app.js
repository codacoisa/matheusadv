(() => {
  ...
  const app = document.querySelector("#generic-app");
  ...
  function itemsTable(detailed) {
    ...
    return `<div class="generic-items"><div class="item-header">${labels.map((label) => `<span>${label}</span>`).join("")}</div>${current.input.items.map((item, index) => itemFields(item, index, detailed)).join("")}</div>`;
  }
  function completeStepOne() { return `<section class="generic-card"><h2>Dados básicos</h2><p class="hint">Identifique o cálculo, o processo e as partes envolvidas.</p>${basicFields(true)}</section>`; }
  function completeStepTwo() { return `<section class="generic-card"><div class="card-title"><div><h2>Parcelas</h2><p class="hint">Cada lançamento pode usar seu próprio índice, taxa e periodicidade.</p></div><button class="secondary" type="button" data-action="add-item">＋ Adicionar parcela</button></div>${itemsTable(true)}<div class="index-actions"><button class="secondary" type="button" data-action="load-indices">${current.indexSnapshot ? "Atualizar índices oficiais" : "Carregar índices oficiais"}</button><span class="hint">Os índices dos itens selecionados serão consultados e preservados no cálculo.</span></div></section>`; }
  ...
  function completeStepThree() {
    const i = current.input;
    return `<section class="generic-card"><div class="extra-section"><div class="card-title"><div><h2>Multas e honorários</h2><p class="hint">Adicione os encargos previstos no título ou contrato.</p></div><div class="button-row"><button class="secondary" type="button" data-action="add-extra" data-kind="penalty">＋ Adicionar multa</button><button class="secondary" type="button" data-action="add-extra" data-kind="fee">＋ Adicionar honorários</button></div></div>${i.penalties.map((item, index) => extraRow("penalty", item, index)).join("") || '<div class="empty-inline">Nenhuma multa adicionada.</div>'}${i.fees.map((item, index) => extraRow("fee", item, index)).join("") || '<div class="empty-inline">Nenhum honorário adicionado.</div>'}</div><div class="extra-section"><div class="card-title"><div><h2>Custas processuais</h2><p class="hint">Atualize as custas com índice próprio quando necessário.</p></div><button class="secondary" type="button" data-action="add-extra" data-kind="cost">＋ Adicionar custas processuais</button></div>${i.costs.map((item, index) => extraRow("cost", item, index)).join("") || '<div class="empty-inline">Nenhuma custa processual adicionada.</div>'}</div><div class="field full">${field("Observações", "notes", i.notes, "text", "full")}</div></section>`;
  }
  function resultStep() {
    const r = current.result;
    if (!r) return '<section class="generic-card"><div class="empty">Calcule os dados para visualizar o resultado.</div></section>';
